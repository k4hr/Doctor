/* path: app/api/question/upload/route.ts */
...
export async function POST(req: Request) {
  try {
    ...
    const q = await prisma.question.findUnique({
      where: { id: questionId },
      select: { id: true, authorTelegramId: true },
    });

    if (!q) return NextResponse.json({ ok: false, error: 'NO_QUESTION' }, { status: 404 });
    if (q.authorTelegramId !== telegramId) return NextResponse.json({ ok: false, error: 'FORBIDDEN' }, { status: 403 });
    ...
    if (photos.length > MAX_PHOTOS) {
      return NextResponse.json({ ok: false, error: 'TOO_MANY_FILES', field: 'photos', max: MAX_PHOTOS }, { status: 400 });
    }
    ...
    for (const f of photos) {
      ...
      if (!isImageMime(mime)) {
        return NextResponse.json({ ok: false, error: 'BAD_MIME', field: 'photos', mime }, { status: 400 });
      }
      if (f.size > maxPhoto) {
        return NextResponse.json({ ok: false, error: 'FILE_TOO_LARGE', field: 'photos', maxBytes: maxPhoto, gotBytes: f.size }, { status: 400 });
      }
    }
    ...
    for (let i = 0; i < photos.length; i++) {
      const f = photos[i];
      const mime = (f.type || '').trim();
      const key = `questions/${questionId}/photo-${Date.now()}-${i + 1}-${rand()}.${safeExt(mime)}`;
      ...
      try {
        ...
        console.error('R2 upload question photo failed', { i, mime, size: f.size, key, details }, e);
    ...
    await prisma.questionFile.createMany({
      data: uploaded.map((x) => ({
        questionId,
        kind: 'PHOTO',
        url: x.url,
    ...
    return NextResponse.json({ ok: true, questionId, photos: uploaded.map((x) => x.url) });
  } catch (e: any) {
    console.error(e);
