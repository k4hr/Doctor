/* path: app/api/doctor/consultations/[id]/decline/route.ts */
...
export async function POST(req: Request, ctx: { params: { id: string } }) {
  try {
    ...
    if (st === 'DECLINED') return NextResponse.json({ ok: true, status: 'DECLINED' }, { headers: { 'Cache-Control': 'no-store' } });
    if (st !== 'PENDING') {
      return NextResponse.json({ ok: false, error: 'BAD_STATUS', hint: `Нельзя отказать из статуса ${st}` }, { status: 409 });
    }
    ...
    return NextResponse.json({ ok: true, status: 'DECLINED' }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (e: any) {
    console.error(e);
