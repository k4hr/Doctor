...
export async function POST(req: Request, ctx: { params: { id: string } }) {
  try {
    ...
    if (String(c.status) !== 'ACCEPTED') {
      return NextResponse.json({ ok: false, error: 'CHAT_LOCKED', hint: 'Чат доступен только после принятия' }, { status: 409 });
    }
    if (!c.paidAt) {
      return NextResponse.json({ ok: false, error: 'NOT_PAID', hint: 'Сначала оплатите консультацию' }, { status: 409 });
    }
