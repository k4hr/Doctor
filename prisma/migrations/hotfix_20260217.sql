-- prisma/hotfix_20260217.sql
-- Цель: гарантировать, что все объекты из 20260217_consultations_and_thanks реально существуют.
-- Это нужно, чтобы потом отметить миграцию как APPLIED через prisma migrate resolve.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = current_schema() AND indexname = 'Doctor_consultationEnabled_idx'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes
    WHERE schemaname = current_schema() AND indexname = 'Doctor_thanksEnabled_idx'
  ) THEN
    CREATE INDEX "Doctor_thanksEnabled_idx" ON "Doctor" ("thanksEnabled");
  END IF;
END$$;

-- 1) ENUM types (создаём как новые, без ADD VALUE)
DO $$
BEGIN
...
-- 2) Таблицы Consultation / ConsultationFile / ConsultationMessage / Thanks
...
-- 3) FK constraints (если нет — добавляем)
DO $$
BEGIN
...
-- 4) UNIQUE paymentId (если не null)
DO $$
BEGIN
