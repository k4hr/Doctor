...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'Consultation_doctorId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Consultation_doctorId_idx'
  ) THEN
    CREATE INDEX "Consultation_doctorId_idx" ON "Consultation" ("doctorId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Consultation_authorTelegramId_idx'
  ) THEN
    CREATE INDEX "Consultation_authorTelegramId_idx" ON "Consultation" ("authorTelegramId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Consultation_status_idx'
  ) THEN
    CREATE INDEX "Consultation_status_idx" ON "Consultation" ("status");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Consultation_createdAt_idx'
  ) THEN
    CREATE INDEX "Consultation_createdAt_idx" ON "Consultation" ("createdAt");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Consultation_doctorId_status_createdAt_idx'
  ) THEN
    CREATE INDEX "Consultation_doctorId_status_createdAt_idx" ON "Consultation" ("doctorId","status","createdAt");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Consultation_authorTelegramId_status_createdAt_idx'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ConsultationFile_consultationId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationFile_consultationId_idx'
  ) THEN
    CREATE INDEX "ConsultationFile_consultationId_idx" ON "ConsultationFile" ("consultationId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationFile_kind_idx'
  ) THEN
    CREATE INDEX "ConsultationFile_kind_idx" ON "ConsultationFile" ("kind");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationFile_consultationId_kind_idx'
  ) THEN
    CREATE INDEX "ConsultationFile_consultationId_kind_idx" ON "ConsultationFile" ("consultationId","kind");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationFile_consultationId_kind_sortOrder_idx'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ConsultationMessage_consultationId_fkey'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ConsultationMessage_authorDoctorId_fkey'
  ) THEN
    ALTER TABLE "ConsultationMessage"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationMessage_consultationId_idx'
  ) THEN
    CREATE INDEX "ConsultationMessage_consultationId_idx" ON "ConsultationMessage" ("consultationId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationMessage_authorDoctorId_idx'
  ) THEN
    CREATE INDEX "ConsultationMessage_authorDoctorId_idx" ON "ConsultationMessage" ("authorDoctorId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationMessage_authorTelegramId_idx'
  ) THEN
    CREATE INDEX "ConsultationMessage_authorTelegramId_idx" ON "ConsultationMessage" ("authorTelegramId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ConsultationMessage_consultationId_createdAt_idx'
  ) THEN
...
-- уникальность paymentId (если не null)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'Thanks_paymentId_key'
  ) THEN
    ALTER TABLE "Thanks"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Thanks_doctorId_idx'
  ) THEN
    CREATE INDEX "Thanks_doctorId_idx" ON "Thanks" ("doctorId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Thanks_doctorId_createdAt_idx'
  ) THEN
    CREATE INDEX "Thanks_doctorId_createdAt_idx" ON "Thanks" ("doctorId","createdAt");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Thanks_authorTelegramId_idx'
  ) THEN
    CREATE INDEX "Thanks_authorTelegramId_idx" ON "Thanks" ("authorTelegramId");
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'Thanks_status_idx'
  ) THEN
